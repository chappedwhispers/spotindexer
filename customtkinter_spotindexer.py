...
from customtkinter import DISABLED, E, END, N, NO, NORMAL, S, W
...
def search_spotify():
    ...
    if not query:
        messagebox.showwarning('Attention', 'Please enter a query search.')
        return
    try:
        ...
        messagebox.showerror('Error', f'Error during Spotify search: {str(e)}')
...
def download_music():
    ...
    if not selected_indices:
        messagebox.showwarning('Attention', 'Please select the track or album to download.')
        return
    ...
    def run_download():
        try:
            ...
            write_to_console(f'Error during the download: {str(e)}\n')
...
def download_album(album_link):
    try:
        ...
        write_to_console(f"Error during the album download: {str(e)}\n")
...
def run_command(command):
    ...
    for line in process.stderr:
        write_to_console(line)
    process.wait()
    write_to_console(f'Finished command with the output code {process.returncode}\n')
...
label_search = ctk.CTkLabel(frame_search, text='Search :')
label_search.grid(row=0, column=0, padx=10, pady=5)
entry_search = ctk.CTkEntry(frame_search, width=130)
entry_search.grid(row=0, column=1, padx=10, pady=5)
search_type_var = tk.StringVar()
search_type_var.set('All')
label_search_type = ctk.CTkLabel(frame_search, text='Search type:')
...
button_search = ctk.CTkButton(frame_search, text='Search', command=search_spotify)
...
frame_format = ctk.CTkFrame(root)
frame_format.pack(pady=10)
format_var = tk.StringVar()
format_var.set('MP3 192kbps')
label_format = ctk.CTkLabel(frame_format, text='Outpout format:')
...
button_download = ctk.CTkButton(root, text='Download', command=download_music)
